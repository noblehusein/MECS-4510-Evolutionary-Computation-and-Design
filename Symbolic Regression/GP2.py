...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 26 02:07:52 2021
...
ops = ['+', '-' ,'*' ,'/' ,'sin' ,'cos'] 
val=[1,2,3,4,5,6,7,8,9,'x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x','x']
...
while iters<3000:
    ...
    if (iters % 10) == 0:
        ...
        plt.savefig('Gif2/image'+str(iters))
        plt.clf()
...
import csv 
with open('Y-values2.csv', 'w') as f: 
    write = csv.writer(f) 
    for val in YY1:
        write.writerow([val])
        
with open('Func2.csv', 'w') as f: 
    write = csv.writer(f) 
    for val in j[0]:
        write.writerow([val])
        
with open('Accuracy2.csv', 'w') as f: 
    write = csv.writer(f) 
    for val in accuracy:
        write.writerow([val])
        
with open('Fitness2.csv', 'w') as f: 
    write = csv.writer(f) 

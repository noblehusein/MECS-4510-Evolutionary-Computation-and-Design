...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 26 02:08:14 2021
...
while iters<3000:
    ...
    if (iters % 300) == 0:
        k=np.array(fitlist)
        firsthalf=k[0:63,0]
        secondhalf=k[64:127,0]     
        diversity = np.mean(abs(firsthalf-secondhalf))
        diversiti.append(diversity)
    ...
    if (iters % 10) == 0:
        ...
        plt.savefig('Gif3/image'+str(iters))
        plt.clf()
...
for i in range(0,128,6):
    plt.scatter(range(0,3000),A[:,i],s=1)
plt.ylim(0,10)
...
with open('Y-values3.csv', 'w') as f: 
    write = csv.writer(f) 
    for val in YY1:
        write.writerow([val])
        
with open('Func3.csv', 'w') as f: 
    write = csv.writer(f) 
    for val in j[0]:
        write.writerow([val])
        
with open('Accuracy3.csv', 'w') as f: 
    write = csv.writer(f) 
    for val in accuracy:
        write.writerow([val])
        
with open('Fitness3.csv', 'w') as f: 
    write = csv.writer(f) 
